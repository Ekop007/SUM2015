 ...
 */
static VOID AK1_SaveLog( CHAR *Text )
{
  FILE *F;
 ...
 */
static CHAR * AK1_TextLoad( CHAR *FileName )
{
  FILE *F;
  CHAR *mem = NULL;
...
UINT AK1_ShaderLoad( CHAR *FileNamePrefix )
{
  ...
  for (i = 0; i < NumOfShaders; i++)
  {
    ...
    {
      isok = FALSE;
      AK1_SaveLog("Error create shader");
      break;
    }
    /* загружаем файл */
    if ((txt = AK1_TextLoad(Buf)) == NULL)
    {
      isok = FALSE;
      AK1_SaveLog("Error load file");
      AK1_SaveLog(Buf);
      break;
    }
    /* закрепляем текст */
...
} /* End of 'AK1_ShaderFree' function */
